...
try:
    ...
    from oaktree.core.utils.namedtuple import namedtuple
...
def get_namedtuple_choices(name, choices_tuple):
    ...
    class Choices(namedtuple(name, [name for val,name,desc in choices_tuple])):
        ...
        def get_val_by_name(self, _name, default=None):
        	for c in tuple([(val,name,desc) for val,name,desc in choices_tuple]):
        		if c[1] == _name:
        			return c[0]
        	return default
